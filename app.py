...
def get_video_info():
    ...
    cookie_index = 0  # Start with the first cookie file
    max_cookies = len(cookie_files)  # Number of available cookies
    success = False

    while not success and cookie_index < max_cookies:
        try:
            # Define yt-dlp options with the current cookie file
            ydl_opts = {
                'noplaylist': True,
                'quiet': True,
                'cookiefile': cookie_files[cookie_index],  # Use the current cookie file
                'oauth2_token': OAUTH_FILE  # OAuth token file path
            }
            ...
            error_message = str(e)
            app.logger.error(f"Error with cookie file {cookie_files[cookie_index]}: {error_message}")

            # Rotate to the next cookie file if error is bot-related
            if "Sign in to confirm you’re not a bot" in error_message:
                app.logger.warning(f"Bot verification error detected with {cookie_files[cookie_index]}. Trying next cookie file.")
                cookie_index += 1  # Move to the next cookie file
            else:
                # Raise the exception if it's unrelated to bot verification
                return jsonify({'error': str(e)}), 500

    # If all cookies are exhausted and download failed
    return jsonify({'error': "All cookie files exhausted. Unable to retrieve video info due to bot verification."}), 500
