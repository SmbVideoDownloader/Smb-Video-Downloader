...
def download_media(url: str) -> Tuple[str, str]:
    ...
    try:
        ...
        # Temporary file path for the merged output
        temp_file_path = tempfile.NamedTemporaryFile(delete=False, suffix=".mp4").name

        # Update the output template for ydl
        ydl_opts['outtmpl'] = f'/tmp/{video_id}.f%(format_id)s.%(ext)s'  # Use a specific naming format

        # Actually download and merge video/audio
        with YoutubeDL(ydl_opts) as ydl_inner:
            ydl_inner.download([url])

        # Check for the existence of the expected temporary files before merging
        video_file = f"/tmp/{video_id}.f303.webm"  # Adjust based on expected format
        audio_file = f"/tmp/{video_id}.f251.webm"  # Adjust based on expected format

        if os.path.exists(video_file) and os.path.exists(audio_file):
            # Proceed to merge the video and audio files
