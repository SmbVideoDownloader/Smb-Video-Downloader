...
def download():
    ...
    try:
        ...
        ydl_opts = {
            'outtmpl': os.path.join(SERVER_DOWNLOAD_DIR, '%(id)s.%(ext)s'),  # Only use video ID and extension
        ...
        return send_file(file_name, as_attachment=True, download_name=os.path.basename(file_name))
