...
def download_media(url: str, format_type: str) -> Tuple[str, str]:
    ...
    ydl_opts = {
        ...
        'outtmpl': os.path.join(DOWNLOAD_FOLDER, '%(id)s.%(ext)s'),  # Set output template to use the download folder
    }
    ...
    try:
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            temp_file_path = os.path.join(DOWNLOAD_FOLDER, filename)  # Ensure full path for temp file

        # Perform the download to the specified temp file path
        ydl_opts['outtmpl'] = temp_file_path  # Set the output template to the temp file path
