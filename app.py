...
def download():
    ...
    url = data.get('url')
    format_type = 'video'  # You can make this dynamic if needed

    if not url:
        return jsonify({'error': 'No URL provided'}), 400

    try:
        # Call the download_media function to get the file data and name
        file_data, file_name = download_media(url, format_type)
