...
def run_yt_dlp_command(url):
    try:
        # Command for yt-dlp, ensure paths are correct
        command = [
            'env/bin/yt-dlp',  # Adjust this to the correct path of yt-dlp
            '--username', 'oauth2',
            '--password', '',
            url
        ]
...
def get_video_info():
    ...
    try:
        # Use yt-dlp to download video using cookies
        ...
        ydl_opts = {'noplaylist': True, 'quiet': True, 'cookiefile': '/var/www/v2mp4.com/cookies/cookies.txt'}
        with YoutubeDL(ydl_opts) as ydl:
