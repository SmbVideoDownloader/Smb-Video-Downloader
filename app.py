...
def download():
    ...
    try:
        # Command to download video with yt-dlp using cookies and OAuth2
...
def run_yt_dlp_command(url):
    try:
        ...
        command = [
            ...
            '--cookies', '/path/to/your/cookies.txt',  # Replace with the correct cookies path
...
def get_video_info():
    ...
    try:
        ...
        ydl_opts = {
            'noplaylist': True,
            'quiet': True,
            'cookiefile': '/path/to/your/cookies.txt'  # Path to cookies file
        }
        with YoutubeDL(ydl_opts) as ydl:
