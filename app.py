...
def download():
    ...
    try:
        # Setting up the download options to select the best available video format
        ydl_opts = {
            'format': 'bestvideo/best',  # This will choose the best video available
            'outtmpl': os.path.join(DOWNLOAD_DIR, '%(title)s.%(ext)s'),  # Path to save the file
            'cookiefile': '/var/www/v2mp4.com/cookies/cookies.txt'  # Path to your cookies.txt file
        }
