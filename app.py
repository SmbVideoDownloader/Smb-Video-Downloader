...
def get_video_info():
    ...
    try:
        # Ensure to specify the full path to yt-dlp and the cookie file
        command = [
            'env/bin/yt-dlp',  # Adjust this path if yt-dlp is located elsewhere
            '--cookies', 
            '/var/www/v2mp4.com/cookies/cookies.txt',  # Path to the cookie file
            url
        ]
        ...
        app.logger.debug("STDOUT: %s", result.stdout)
        app.logger.debug("STDERR: %s", result.stderr)

        # Check if the command was successful
        if result.returncode != 0:
            # Parse the error message more cleanly
