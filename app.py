...
def download():
    ...
    try:
        # Use OAuth2 to download the video with the same command
        ...
        file_name = output_lines[-1] if output_lines else "Downloaded file"  # Adjust based on the actual output format
...
def run_yt_dlp_command(url):
    try:
        # Command for yt-dlp with OAuth2 parameters for downloading
        command = [
