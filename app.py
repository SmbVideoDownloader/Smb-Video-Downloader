...
def download_media(url: str, format_type: str) -> str:
    ...
    # Select a proxy from the list that is reachable
    reachable_proxies = [proxy for proxy in PROXIES if check_proxy(proxy)]
    if reachable_proxies:
        ydl_opts['proxy'] = random.choice(reachable_proxies)
    else:
        logging.error("No reachable proxies available.")
        raise Exception("All configured proxies are unreachable.")
