...
def download_media(url: str) -> Tuple[BytesIO, str, str]:
    """Download media with video and AAC audio (not Opus) and return the data in-memory (BytesIO), filename, and file path."""

    ydl_opts = {
        'format': 'bestvideo+bestaudio/best',  # Fall back to the best single stream if no separate video/audio available
        ...
        'postprocessors': [
            {
                'key': 'FFmpegExtractAudio',  # Extract audio to AAC format if separate audio is present
                'preferredcodec': 'aac',      # Set the preferred audio codec to AAC
            }
        ],
    ...
    try:
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            if not info_dict.get('formats'):
                raise Exception("No available formats found for the video.")
        ...
        if os.path.exists(merged_file_path) and os.path.getsize(merged_file_path) > 1024:
            ...
            video_data.seek(0)
            return video_data, filename, merged_file_path  # Return file path for cleanup
...
def download():
    ...
    try:
        ...
        video_data, filename, merged_file_path = download_media(url)

        @after_this_request
        def cleanup(response):
            # Clean up the merged file after the response is sent
            try:
                os.remove(merged_file_path)
                logging.info(f"Deleted temporary file: {merged_file_path}")
            except Exception as e:
                logging.error(f"Error deleting file {merged_file_path}: {str(e)}")
            return response
